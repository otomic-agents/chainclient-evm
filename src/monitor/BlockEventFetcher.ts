...
export default class BlockEventFetcher {
    ...
    constructor(_monitor: Monitor, historyMode: boolean, historyModeEndBlock: number | undefined) {
    ...
    startFetch = async () => {
        ...
        if (this.monitor.taskBlockEventNow == undefined) {
            ...
            if (this.monitor.evmConfig == undefined) throw new Error("evmConfig not found");
        ...
        let dispatch = async () => {
            ...
            if (task_number < 10) {
                ...
                await self.blockHeight((err: Error, result: any) => {
                    ...
                    if (!err) {
                        if (self.historyMode) {
                            ...
                            self.monitor.blockHeight = parseInt(result, 16) - 6; //prevent chasing uncles
                        }
                    }

                });

                if (self.monitor.taskBlockEventNow == undefined) throw new Error("block event fetcher state error: monitor.taskBlockEventNow undefined");
                if (self.monitor.blockHeight == undefined) throw new Error("block event fetcher state error: monitor.blockHeight undefined");
                ...
                while (
                    ...
                    if (block_end == undefined) throw new Error("block event fetcher state error: block_end undefined");
    ...
    blockHeight = async (callback: Function) => {
        ...
        try {
            ...
            result = await this.monitor.evmRpcClient
                .get()
                .request(
                    { 
                        // jsonrpc: "2.0", 
